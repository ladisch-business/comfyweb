...
<div class="max-w-4xl mx-auto">
    ...
    <form action="{{ route('workflows.update', $workflow) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" 
                              class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $workflow->description) }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="json_content" id="json_content" rows="10" 
                              ...
                              required>{{ old('json_content', $workflow->json_content) }}</textarea>
                    @error('json_content')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="field_config" id="field_config" rows="3" 
                              class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 font-mono text-sm"
                              required>{{ old('field_config', json_encode($workflow->field_config)) }}</textarea>
                    @error('field_config')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="supports_lora" id="supports_lora" value="1" 
                           {{ old('supports_lora', $workflow->supports_lora) ? 'checked' : '' }}
